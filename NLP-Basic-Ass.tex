...
%\item An end-of-sentence period ({\tt .}) has been inserted in two places -- can you find them?
\end{itemize}
...
%In addition, there are two pairs of round brackets in the regular expression, one enclosing the entire disjunction, 
%and one enclosing an empty string at the end of the second part. These brackets are completely superfluous as far as the matching is concerned. They are only there to guarantee 
%that the behavior of the program will not change if you later feel the need to add your own brackets for grouping. (Unless you are especially interested in the inner workings of Python, 
%you can completely ignore this technicality for now as long as you don't remove the brackets.)

%\paragraph{Note on Python versions:} In this and following exercises, we will use version 3 of Python because it assumes the Unicode text encoding as default. On most systems,
%this means that you should type {\tt python3}, instead of simply {\tt python}, when running a Python program. Make sure that you can get {\tt tokenizer1.py} to run on your system
%before proceeding further and consult the teacher otherwise.
